...
echo '<div class="most-read-news">';
    echo '<ul class="nav nav-pills text-center" role="tablist">';
        echo '<li role="presentation" class="active"><a href="#days7" aria-controls="days7" role="tab" data-toggle="tab">'.$locale['MRN_02'].'</a></li>';
        echo '<li role="presentation"><a href="#days14" aria-controls="days14" role="tab" data-toggle="tab">'.$locale['MRN_03'].'</a></li>';
        echo '<li role="presentation"><a href="#days30" aria-controls="days30" role="tab" data-toggle="tab">'.$locale['MRN_04'].'</a></li>';
    echo '</ul>';

    echo '<div class="tab-content m-t-5">';
        echo '<div role="tabpanel" class="tab-pane active" id="days7">';
            $result_7days = dbquery("SELECT news_id, news_subject, news_reads, news_datestamp, news_start, news_end, news_draft
                FROM ".DB_NEWS."
                WHERE (".time()." > news_start OR news_start = 0) AND news_draft = 0 AND (".time()." < news_end OR news_end = 0) AND news_draft = 0 AND news_datestamp > '".strtotime('-7 day')."'
            ...
            if (dbrows($result_7days) > 0) {
                ...
                while ($data = dbarray($result_7days)) {
                    echo '<li><a href="'.INFUSIONS.'news/news.php?readmore='.$data['news_id'].'">'.$data['news_subject'].'</a> <span class="badge">'.$data['news_reads'].' <i class="fa fa-eye"></i></span></li>';
                }
                echo '</ul>';
            } else {
        ...
        echo '<div role="tabpanel" class="tab-pane" id="days14">';
            $result_14days = dbquery("SELECT news_id, news_subject, news_reads, news_datestamp, news_start, news_end, news_draft
                FROM ".DB_NEWS."
                WHERE (".time()." > news_start OR news_start = 0) AND news_draft = 0 AND (".time()." < news_end OR news_end = 0) AND news_draft = 0 AND news_datestamp > '".strtotime('-14 day')."'
                ".(multilang_table('NS') ? " AND news_language='".LANGUAGE."' AND " : '').groupaccess('news_visibility')."
                ORDER BY news_reads DESC
                LIMIT 15
            ");
        ...
        echo '<div role="tabpanel" class="tab-pane" id="days30">';
            $result_30days = dbquery("SELECT news_id, news_subject, news_reads, news_datestamp, news_start, news_end, news_draft
                FROM ".DB_NEWS."
                WHERE (".time()." > news_start OR news_start = 0) AND news_draft = 0 AND (".time()." < news_end OR news_end = 0) AND news_draft = 0 AND news_datestamp > '".strtotime('-30 day')."'
                ".(multilang_table('NS') ? " AND news_language='".LANGUAGE."' AND " : '').groupaccess('news_visibility')."
                ORDER BY news_reads DESC
                LIMIT 15
            ");
